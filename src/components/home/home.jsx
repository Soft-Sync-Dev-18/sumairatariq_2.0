...
export default function Home() {
  ...
  return (
    <main className="min-h-screen">
      {loading ? (
        <div>
          <Skeleton className="w-full mx-auto md:aspect-[9/3.5] aspect-[9/8] bg-slate-200 inline-block object-cover object-center " />
        </div>
      ) : (
                ...
                {Array(4)
                  ...
                  .map((_, index) => (
                    <div key={index} className="flex flex-col space-y-2">
                      <Skeleton
                        className="  md:h-[360px] h-80 w-full rounded-[4px] bg-[#CCCCCC]"
                        animation="wave"
                      />
                    </div>
                  ))}
